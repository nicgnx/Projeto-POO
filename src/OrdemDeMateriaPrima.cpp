...
// Métodos
Fornecedor* OrdemDeMateriaPrima::melhorPreco(
    int idMateriaPrima, std::vector<Fornecedor*> fornecedores
) {
    ...
    if (auxFornecedores.size() <= 0) {
        ...
        this->valorFinal = this->quantidade *
                           fornecedor->enviaPreco(this->materiaPrima->getId());
        return fornecedor;
    }
}
