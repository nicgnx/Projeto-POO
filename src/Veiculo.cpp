...
Veiculo::Veiculo(int capacidade,Turno* turno,vector<Funcionario*> rotasFuncionarios){
...
void Veiculo::verificaTurno(){
  ...
  for(int i = 0;i < this->rotasFuncionarios.size();i++){
    if(this->rotasFuncionarios[i]->getTurno()->getPeriodo() != this->turno->getPeriodo()){}
      else aux.push_back(this->rotasFuncionarios[i]);
  }
