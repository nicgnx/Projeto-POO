  ...
  if (verificaEstoqueProduto(idProduto) > this->produto[idProduto]->getEstoqueMinimo()) {
    ...
    if (this->produto[idProduto]->getEstoqueMinimo() > this->produto[idProduto]->getLoteMinimo()) {
      ...
      OrdemDeProducao Solicitacao(
        this->produto[idProduto]->getNome(), idProduto, Data().dateNow(),
...
void Estoque::reabasteceEstoqueMP(int idMateriaPrima) {
  ...
  if (verificaEstoqueMP(idMateriaPrima) > this->estoqueMP[idMateriaPrima]) {
    ...
    OrdemDeMateriaPrima Solicitacao(
      ...
      this->materiaPrima[idMateriaPrima]->getEstoqueMinimo(), Data().dateNow(),
      this->fornecedores
    );
...
void Estoque::printListaDeProdutos() {
  std::cout << "---------------------------------------------------------------"
               "-------------------------------------\n\n";
  std::cout << "                            Lista dos Produtos Cadastrados no "
               "Sistema\n\n";
  for (std::map<int, Produto*>::iterator it = produto.begin();
    ...
    std::cout << "Produto: " << this->produto[it->first]->getNome()
              ...
              << "     Quantidade: "
              << verificaEstoqueProduto(this->produto[it->first]->getIdProduto()
                 )
              << "\n";
  }
  std::cout << "\n\n-----------------------------------------------------------"
  ...
  switch (mens) {
    case N_REALIZADO: {
      ...
      std::cout << "                  O estoque minímo do produto [ "
                << this->produto[id]->getNome() << " ] ainda não foi atingido.";
    ...
    case N_REALIZADO_MP: {
      ...
      std::cout << "                  Solicitação de Reabastecimento de "
                   "Estoque "
                   "não realizada.\n\n";
      std::cout << "                  O estoque minímo da Materia Prima [ "
